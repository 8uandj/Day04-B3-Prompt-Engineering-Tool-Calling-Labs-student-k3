import { PanelLeftOpen, ShieldCheck, Wrench, MessagesSquare, Activity, Languages } from "lucide-react";
...
export default function App() {
  ...
  const [config, setConfig] = useState<Config | null>(null); const [session, setSession] = useState<Session | null>(null); const [live, setLive] = useState<StreamEvent[]>([]); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [mobileTab, setMobileTab] = useState<"chat" | "trace">("chat");
  ...
  const send = async (content: string) => { setError(""); setLive([]); setBusy(true); try { const active = session ?? await createSession(settings); if (!session) setSession(active); await streamMessage(active.transcript_id, content, (event) => { if (event.type === "stream_closed" && event.session) { setSession(event.session); setLive([]); } else setLive((items) => [...items, event]); }); } catch (reason) { setError(`${t.streamError} ${String(reason)}`); } finally { setBusy(false); } };
  ...
  return <div className={`app-shell app-shell--${sidebar}`}><Sidebar config={config} copy={t} language={language} mode={sidebar} settings={settings} session={session} onMode={setSidebar} onSettings={setSettings} onLanguage={() => setLanguage(language === "vi" ? "en" : "vi")} onNew={newChat}/>
    ...
    <main className="main-content"><header className="topbar"><div className="topbar-title"><span className="live-dot"/><div><strong>Research Command Center</strong><small><ShieldCheck/>Evidence mode</small></div></div><div className="topbar-actions"><button className="language-mobile" onClick={() => setLanguage(language === "vi" ? "en" : "vi")}><Languages/><span>{language.toUpperCase()}</span></button><HistoryDialog copy={t} onSelect={restore}/><span className="artifact-chip">{artifact}</span></div></header>
      ...
      <div className={`workspace mobile-tab--${mobileTab}`}><ChatPanel copy={t} session={session} busy={busy} onSend={send}/><TracePanel copy={t} session={session} live={live}/></div>
    </main>
  </div>;
}
